...
export default function CellProductByTruck({ product, length }: { product: Product, length: number }) {

    
    const productService = ProductServices.getInstance();
    const [canShowModal, setCanShowModal] = useState(false);
    const result = calculateMaxProducts(product);
    const finalPrice : number = calculatePrice(product, length, product.operationCoust);
     return (
    <CellBody>
